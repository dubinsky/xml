...
final class HtmlXmlWriterConfigSpec extends AnyFunSuite:
  private def render(element: ZioBlocksXml.Element, width: Int = 40): String =
  ...
  test("span with two element children is not indented (no HTML space inside)") {
    val ref: ZioBlocksXml.Element = ZioBlocksXml
      .element(XmlElement.Span)
      .addClass("glossary-ref")
      .setChildren(Chunk(
        ZioBlocksXml.element(XmlElement.A).setHref("#posuk").setText("posuk"),
        ZioBlocksXml.element(XmlElement.Span).addClass("glossary-tip").setText("verse")
      ))
    val paragraph: ZioBlocksXml.Element = ZioBlocksXml
      .element(XmlElement.P)
      .setChildren(Chunk(ZioBlocksXml.text("("), ref, ZioBlocksXml.text(" 1)")))
  ...
  test("void elements self-close; empty non-void elements do not") {
    assert(render(ZioBlocksXml.element(XmlElement.Br)).contains("<br/>"))
    assert(render(ZioBlocksXml.element(XmlElement.Img).set(XmlAttribute.Src, "x")).contains("/>"))
    val script: String = render(ZioBlocksXml.element("script"))
  ...
  test("br with following whitespace forces a newline") {
    val xml: ZioBlocksXml.Element = XmlParser.parseXml("<p>foo<br/>\nbar</p>").toOption.get
  ...
  test("br without following whitespace stays inline") {
    val xml: ZioBlocksXml.Element = XmlParser.parseXml("<p>foo<br/>bar</p>").toOption.get
    val dumped: String = render(xml, width = 120)
    assert(dumped.contains("foo<br/>bar"), dumped)
  }

  test("span still preserves a real space before an inner element") {
    val span: ZioBlocksXml.Element = ZioBlocksXml
      .element(XmlElement.Span)
      .setChildren(Chunk(ZioBlocksXml.text("foo "), ZioBlocksXml.element(XmlElement.Em).setText("bar")))
  ...
  test("adjacent phrasing children are not stacked") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.P).setChildren(Chunk(
        ZioBlocksXml.element(XmlElement.Em).setText("a"),
        ZioBlocksXml.element("strong").setText("b")
      )),
      width = 120
    )
  ...
  test("div with two paragraphs still stacks") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Div).setChildren(Chunk(
        ZioBlocksXml.element(XmlElement.P).setText("a"),
        ZioBlocksXml.element(XmlElement.P).setText("b")
      )),
      width = 120
    )
  ...
  test("script text keeps < and &&") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Script).setText("if (a < b && c) {}"))
  ...
  test("script newlines and // comments survive wrapping") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Script).setText("a();\n// comment\nb();")
    )
  ...
  test("long script string with spaces is not wrapped") {
    ...
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Script).set("type", "application/ld+json").setText(json)
    )
  ...
  test("script source </script> is broken") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Script).setText("a</script>b"))
  ...
  test("style body </style> is broken") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Style).setText("a</style>b"))
    assert(dumped.contains("""a<\/style>b"""), dumped)
    assert(!dumped.contains("a</style>b"), dumped)
  }

  test("style keeps > and <") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Style).setText("""a > b; content:"<""""))
  ...
  test("module and json-ld scripts use the same raw rules") {
    ...
    val jsonLd: String = render(
      ZioBlocksXml.element(XmlElement.Script).set("type", "application/ld+json").setText("a < b")
    )
    assert(jsonLd.contains("a < b"), jsonLd)
  }

  test("pre still encodes <") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Pre).setText("a < b"))
    assert(dumped.contains("a &lt; b"), dumped)
  }

  test("root pre keeps an embedded newline") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Pre).setText("a\nb"))
    assert(dumped.contains("a\nb"), dumped)
  ...
  test("script inside pre is raw") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Pre).setChildren(Chunk(
        ZioBlocksXml.text("a < b"),
        ZioBlocksXml.element(XmlElement.Script).setText("c < d"),
        ZioBlocksXml.text("e < f")
      ))
    )
    assert(dumped.contains("a &lt; b"), dumped)
  ...
  test("nested script child end tag is broken; outer end tag is real") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Script).setChildren(Seq(
        ZioBlocksXml.element(XmlElement.Script).setText("x")
      ))
    )
  ...
  test("pre inside script is serialized as tags and not encoded") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Script).setChildren(Seq(
        ZioBlocksXml.element(XmlElement.Pre).setText("a < b")
      ))
    )
  ...
  test("adjacent script text children concatenate") {
    val xml: String = render(
      ZioBlocksXml.element(XmlElement.Script).setChildren(Seq(ZioBlocksXml.text("a"), ZioBlocksXml.text("b")))
    )
  ...
  test("onclick still encodes <") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Div).set("onclick", "a < b"))
    assert(dumped.contains("a &lt; b"), dumped)
  }

  test("title and textarea bodies still encode <") {
    val title: String = render(ZioBlocksXml.element(XmlElement.Title).setText("a < b"))
    assert(title.contains("a &lt; b"), title)
    val textarea: String = render(ZioBlocksXml.element("textarea").setText("a < b"))
    assert(textarea.contains("a &lt; b"), textarea)
  }

  test("empty script keeps an end tag") {
    val dumped: String = render(ZioBlocksXml.element(XmlElement.Script))
  ...
  test("root script is not mixed-tokenized") {
    val dumped: String = render(
      ZioBlocksXml.element(XmlElement.Script).setText("foo bar baz quux"),
      width = 8
    )
